...
应用启动后会检查已签名的更新清单。发现新版本时，可在版本弹窗中直接下载、验证、安装并重启，不再需要通过浏览器手动下载安装包。每个更新包都会先使用应用内置的公钥验证发布签名。

安装仍需用户确认，只有点击「下载并安装」后才会开始下载。Linux `.deb` 更新会通过操作系统请求管理员权限；AppImage 更新要求当前文件具有写入权限。

`v0.2.1` 及更早版本只包含浏览器下载流程，需要手动安装一次 `v0.2.2`。从该桥接版本开始，后续版本即可使用应用内更新。
...
维护者签名与发布流程参见 [`docs/RELEASING.md`](docs/RELEASING.md)。
